...
include("function.php");
  ...
  *****************************************/
  ...
  elseif((!empty($_POST['name'])) AND (!empty($_POST['email'])) AND (!empty($_POST['date_of_birth'])) AND (!empty($_POST['password']))  ) 
    ...
    if (check_existe_email_teacher($_POST['email'])!=1 ) {
      ...
      if(test_email($_POST['email'])==1){
            $query1 = 'INSERT INTO demande_teacher(name,email,date_of_birth,password) values(?,?,?,?)';


            $values1 = array($_POST['name'],$_POST['email'],$_POST['date_of_birth'],$_POST['password']);
