...
if (isset($_POST['name']) && isset($_POST['date_of_birth']) && isset($_POST['email']) && isset($_POST['password'])) {
	...
	// Vérifier si l'email existent déjà
	$query = "SELECT * FROM teacher WHERE email = ?";
	$value = array($email);
	$result = PDO($query, $value);
	if ($result->rowCount() > 0) {
		header("location: ../teacher_trait.php?etat=duplicate");
	}else{

			// Insérer l'enregistrement dans la base de données
			$query = "INSERT INTO teacher(name, date_of_birth, email, password) VALUES (?, ?, ?, ?)";
			...
			if ($result) {
				header("location: ../teacher_trait.php?etat=true");
			} else {
				header("location: ../teacher_trait.php?etat=false");
			}
		}
} else {
	header("location: ../teacher_trait.php?etat=false");
