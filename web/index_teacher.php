...
$query = "SELECT * from cours where teacher_id=?" ;
$value = array($_SESSION['teacher_id']);
$res = PDO($query,$value);
...
<p class="card-text"style="font-size:20pt;border-left-style: solid;border-left-width: 8px; border-left-color: #00A67E;">  <b>'.$nom_cours.'</b></p>
';
// affichage audio
...
echo'<form method="POST" action="traitement/dropcour.php">
                
                  <input type="hidden" name="id" value="'.test_input($row['cours_id']).'">
                  <button type="submit" class="btn btn-outline-danger btnmarging" onclick="return confirm(\'Êtes-vous sûr de vouloir supprimer cet cour ?\')">Supprimer</button>
